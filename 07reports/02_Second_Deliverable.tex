...
\subsection{More about continuous treatment event studies}  The standard binary treatment event study is easily extended to the continuous treatment case.  The following adapts the description from \cite{callaway_2024a} to our purpose.
...
The two parameters of interest then are the average level treatment effects and the average causal responses.  We consider the case where once an individual is treated with a particular dose, they remain treated with that dose thereafter.  Let \textit{g} denote the time the individual becomes treated.
The average treatment effect of dosage \textit{d} is $$ATE(g,t,d) = \mathbb{E}[Y_{kt}(g,d) - Y_{kt}(0)|G=g]$$  and the average causal response is $$ACR(g,t,d) = \frac{\partial ATE(g,t,d)}{\partial d} = \frac{\partial Y_{ktg}}{\partial d}$$ Under some assumptions (discussed in \ref{sec:assumps}), the ATE parameter is identified by $\beta_j$ in the estimating equation below.  
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{06figures/ES_felony_all_basicfe_cts-trt.pdf}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{06figures/ES_felony_all_basicfe_cts-trt.pdf}
...
\newpage
\nocite{callaway_2021, callaway_2025, cjars_data}
\printbibliography
