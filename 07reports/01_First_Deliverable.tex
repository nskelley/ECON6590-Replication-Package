...
Our broad question of interest is ``How does deindustrialization impact crime outcomes?''  Specifically, we plan to examine the collapse of the coal industry in Appalachia, where coal was a significant industry up until the rise of natural gas in the United States in the early 2002's.  
...
\begin{align}
    ...
    Z_{kt} &= \text{coal\_share}_{k,2002} \cdot \text{coal\_growth}_t \tag{2} \\
...
$Z_{kt} = \text{coal\_share}_{k,2002} \cdot \text{coal\_growth}_t$, 
where $\text{coal\_share}_{k,2002}$ is county $k$'s baseline dependence on coal employment 
(measured as the share of total employment in coal mining in the year 2002), and 
$\text{coal\_growth}_t$ is the national growth rate of coal employment in year $t$, 
and the baseline exposure term $\text{coal\_share}_{k,2002}$ is normalized such that 
$\sum_k \text{coal\_share}_{k,2002} = 1$ in each period.
...
\begin{enumerate}
    \item \textbf{Relevance:} $\text{Cov}(Z_{kt}, D_{kt} \mid X_{kt}, \kappa_k, \tau_t) \neq 0.$ The instrument (coal dependence × national growth) actually shifts local coal employment.
    \item \textbf{Exogeneity of shocks:} $\text{coal\_growth}_t$ is uncorrelated with county-level shocks to $\varepsilon_{kt}.$ (i.e. National coal trends aren’t driven by county-level crime or local conditions.)
    ...
    \item \textbf{Monotonicity:} Higher $\text{coal\_growth}_t$ weakly increases $D_{kt}$ for all $k$. When national coal jobs rise, every coal-dependent county moves in the same direction (no “defiers”). This may not hold, but I wonder if the Dechaisemartin (2017) (more compliers than defiers at all times) assumption can help bail us out!
\end{enumerate}

CONNOR's ASSUMPTIONS ABOVE THIS LINE! ADDIE's ARE BELOW THIS LINE
\newline
...
Running a simple OLS regression of county crime rates (Y) on change in coal employment (X), gives us some descriptive summary statistics about how our crime outcome variable is related to changes in coal employment. In particular, we see crime rates increase (decrease) as the change in coal employment for a particular county declines. 

Figure XX bin-scatter (bar chart/line graph) below provides suggestive evidence that counties experiencing a significant drop in coal employment see higher (lower) crime outcomes. 
 [ SUMMARY STATS]
